...
/// fn main() {}
/// ```
pub trait NeuroProblem: Problem + Clone {
...
/*
pub trait NeuroProblemBattle: ProblemBattle {
	/// Number of input variables.
	fn get_inputs_num(&self) -> usize;
	/// Number of output (target) variables.
	fn get_outputs_num(&self) -> usize;
	...
	/// # Arguments:
	/// * `net` - neural network to compute fitness for.
	fn compute_with_net<T: NeuralNetwork>(&self, net1: &mut T, net2: &mut T) -> (f32, f32);
}*/
...
impl NeuroProblem for XorProblem {
	fn get_inputs_num(&self) -> usize { 25 }
	fn get_outputs_num(&self) -> usize { 1 }
	fn get_default_net(&self) -> MultilayeredNetwork {
		let mut rng = rand::thread_rng();
		let mut net: MultilayeredNetwork = MultilayeredNetwork::new(self.get_inputs_num(), self.get_outputs_num());
	...
	fn compute_with_net<T: NeuralNetwork>(&self, nn: &mut T) -> f32 {
		...
		for bit_pattern in 0..total_patterns {
			...
			let output = nn.compute(&bit_array);
			er += (result as f32 - output[0]) * (result as f32 - output[0]);
			// ビットパターンと結果を表示
			//println!("{:?} => {}", bit_array, result);
		}
...
#[allow(dead_code)]
pub struct XorProblem2 {}

#[allow(dead_code)]
impl XorProblem2 {
	pub fn new() -> XorProblem2 {
		XorProblem2 {}
	}
}
...
impl Clone for SymbolicRegressionProblem {
	fn clone(&self) -> Self {
		todo!()
	}
}
