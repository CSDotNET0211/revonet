...
impl<'a, P: Problem + Sync + Send + 'static + Clone> EA<'_, P> for GA<'a, P> {
...
fn cross_blx_alpha<T: Individual, R: Rng + Sized>(p1: &T, p2: &T, c1: &mut T, c2: &mut T, alpha: f32, mut rng: &mut R) {
	...
	for k in 0..gene_count {
		let (min_gene, max_gene) = if p1_genes[k] > p2_genes[k] { (p2_genes[k], p1_genes[k]) } else { (p1_genes[k], p2_genes[k]) };
